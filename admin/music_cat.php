...
function videos_cat_number() {
    ...
    $number = $db->query("SELECT id_cat FROM music_cat");
    $number->execute();
...
function videos_cat() {
    ...
    while($x = $cat->fetch()){
        ...
        $cats .= '
            <li><a href="music_cat_view.php?id='.$x->id_cat.'" title="">'.$x->name.'</a></li>
        ';
    }
...
<article class="center animation fadeInUp">
    <section id="page_right">
        <h3 class="title"><i class="fa fa-clone"></i> Catégories Musiques (<?php echo videos_cat_number(); ?>)<span>Liste des catégories des musiques</span></h3>
        <div id="page_content">
            <ul id="lang_menu">
                <?php echo videos_cat(); ?>
            </ul>
    ...
    <section id="bloc_right">
        <div id="buttons_right">
            <a href="music_cat_add.php" title="" class="btn_right btn_green"><i class="fa fa-plus"></i> Ajouter une catégorie</a>
        </div>
    </section>
</article>
<?php
