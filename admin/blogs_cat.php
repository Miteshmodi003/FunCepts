...
function videos_cat_number() {
    ...
    $number = $db->query("SELECT id_cat FROM blogs_cat");
    $number->execute();
...
function videos_cat() {
    ...
    while($x = $cat->fetch()){
        ...
        $cats .= '
            <li><a href="blogs_cat_view.php?id='.$x->id_cat.'" title="">'.$x->name.'</a></li>
        ';
    }
...
<article class="center animation fadeInUp">
    <section id="page_right">
        <h3 class="title"><i class="fa fa-clone"></i> Catégories Blogs (<?php echo videos_cat_number(); ?>)<span>Liste des catégories des blogs</span></h3>
        <div id="page_content">
            <ul id="lang_menu">
                <?php echo videos_cat(); ?>
            </ul>
